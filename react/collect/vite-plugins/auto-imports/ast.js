...
export const getExportedInfo = (filename) => {
  const results = {
    values: [],
    types: [],
  }
  ...
  const visit = (node, inNamespace = false) => {
...
const generateParameter = (parameter, refer = false) => {
  if (!parameter?.length)
    return ''
...
export const generateType = (target, { name, parameter, alias }) =>
